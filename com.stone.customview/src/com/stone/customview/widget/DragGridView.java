...
public class DragGridView extends GridView {
	...
	private IDragOperation mDragOperation;
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if(adapter instanceof IDragOperation){
			mDragOperation = (IDragOperation) adapter;
		}else{
...
//		System.out.println("numColumns=" + mNumColumns);
	}
...
//		return super.onInterceptTouchEvent(ev);
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			/*
			 * 在AbsListView中，使用getChildAt(index)的取值，只能是当前可见区域（列表可滚动）的子项！ 
			 * 即 AbsListView.getFirstVisiblePosition() <= index <= AbsListView.getLastVisiblePosition(); 
			 */
	...
	private Runnable mScrollRunnable = new Runnable() {
		...
		public void run() {
			...
			/*
			 * mUpScrollBorder 	 是4/5容器高度
			 * mDownScrollBorder 是1/5容器高度
			 */
	...
	private void onSwapItem(int moveX, int moveY){
		...
		if(tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION && mAnimationEnd){
			mDragOperation.reorderItems(mDragPosition, tempPosition);
			mDragOperation.setHideItem(tempPosition);
